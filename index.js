...
let initId = 4
const findIndexId = (idParams) => { return posts.findIndex(p => p.id == Number(idParams)) }
...
app.post("/posts", (req, res) => {
    try {
        ...
        if (typeof subject !== 'string' || subject.trim() == '' ||
            ...
            return res.status(400), json({ message: "subject, desc는 비워지지 않은 문자열 이여야 합니다." })
        }
        const newPost = {
            id: initId++,
...
app.get("/posts/:id", (req, res) => {
    try {
        ...
        res.status(200).json({ message: "게시글 1개 불러오기 완료", posts: posts[index] })
    } catch (error) {
        console.error("게시글 조회 중 오류", error)
        res.status(500).json({ message: "서버 오류" })
    }
...
app.delete("/posts/:id", (req, res) => {
    try {
        ...
        res.status(200).json({ message: "게시글 1개 삭제 완료", posts })
    } catch (error) {
        console.error("게시글 조회 중 오류", error)
        res.status(500).json({ message: "서버 오류" })
    }
